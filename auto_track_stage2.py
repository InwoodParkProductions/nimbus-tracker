...
def _solve_and_finish(clip, ts, mask_stack, settings, stats, fs, fd,
    ...
    if err is None or err > tripod_at:
        # Pick the best-covered frame in each half of the shot.
        #
        # Choosing the pair that shares the MOST tracks is the textbook answer
        # — triangulation needs tracks visible on both keyframes — and it was
        # tried here. It measurably regressed: across the 5-shot A/B it took
        # shot 10 from perspective 5.87px to no solve at all, and the run from
        # 4 wins to 2. Theory lost to measurement; if you try it again, re-run
        # the A/B before keeping it.
        #
        # find_frame() defaults to exact=False and returns the NEAREST marker,
        # so a track whose markers stop at frame 100 answers "present" for
        # frame 700 — this coverage count is generous by design, and tightening
        # it to exact=True was part of what regressed.
        def marker_count(f):
